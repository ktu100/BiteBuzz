...
const OurMenuHome = () => {
  ...
  useEffect(() => {
    axios.get('https://bitebuzz-backend-9ex9.onrender.com/api/items')
      .then(res => {
  ...
  return (
    <div className="bg-gradient-to-br from-rose-300 via-[#2a1e14] to-[#3e2b1d] min-h-screen py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-4xl sm:text-5xl md:text-6xl font-bold text-center mb-12 bg-clip-text text-transparent bg-gradient-to-r from-rose-200 via-rose-300 to-rose-200">
          ...
          {categories.map(cat => (
            <button
              ...
              className={`px-4 sm:px-6 py-2 rounded-full border-2 transition-all duration-300 transform font-cinzel text-sm sm:text-lg tracking-widest backdrop-blur-sm ${
                activeCategory === cat
                  ? 'bg-gradient-to-br from-rose-100 to-rose-700/80 border-rose-800 scale-105 shadow-xl shadow-amber-900/30'
                  : 'bg-amber-900/20 border-amber-800/30 text-amber-100/80 hover:bg-rose-800/40 hover:scale-95'
              }`}
            >
              {cat}
            </button>
        ...
        <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4">
          {displayItems.map((item, i) => {
            ...
            return (
              <div
                key={item._id}
                className="relative bg-amber-900/20 rounded-2xl overflow-hidden border border-rose-800/30 backdrop-blur-sm flex flex-col transition-all duration-500"
                style={{ '--index': i }}
              >
                ...
                <div className="p-4 sm:p-6 flex flex-col flex-grow">
                  ...
                  <h3 className="text-xl sm:text-2xl mb-2 font-dancingscript text-rose-100 transition-colors">
                    {item.name}
                  </h3>
                  <p className="text-rose-100/80 text-xs sm:text-sm mb-4 font-cinzel leading-relaxed">
                    {item.description}
                  </p>
                  ...
                  <div className="mt-auto flex items-center gap-4 justify-between">
                    <div className="bg-amber-100/10 backdrop-blur-sm px-3 py-1 rounded-2xl shadow-lg">
                      <span className="text-xl font-bold text-rose-300 font-dancingscript">
                      ...
                      {qty > 0 ? (
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="w-8 h-8 rounded-full bg-rose-900/40 flex items-center justify-center hover:bg-rose-800/50 transition-colors"
                          >
                            <FaMinus className="text-rose-100" />
                          </button>
                          ...
                          <button
                            ...
                            className="w-8 h-8 rounded-full bg-rose-900/40 flex items-center justify-center hover:bg-amber-800/50 transition-colors"
                          >
                            <FaPlus className="text-rose-100" />
                          </button>
                        </div>
                      ) : (
                        <button
