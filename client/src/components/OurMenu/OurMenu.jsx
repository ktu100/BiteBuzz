...
const OurMenu = () => {
  ...
  useEffect(() => {
    const fetchMenu = async () => {
      try {
        const res = await axios.get('https://bitebuzz-backend-9ex9.onrender.com/api/items');
  ...
  return (
    <div className="bg-gradient-to-br from-[#1a120b] via-[#2a1e14] to-[#3e2b1d] min-h-screen py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <h2 className="text-center mb-12 bg-clip-text text-transparent bg-gradient-to-r from-rose-200 via-rose-300 to-rose-200">
          <span className="font-dancingscript block text-5xl sm:text-6xl md:text-7xl mb-2">
            Our Delicious Menu
          </span>
          <span className="block text-xl sm:text-2xl md:text-3xl font-cinzel mt-4 text-rose-100/80">
            Taste Heaven
          </span>
        </h2>
          ...
          {categories.map(cat => (
            <button
              ...
              className={`px-4 sm:px-6 py-2 rounded-full border-2 transition-all duration-300 transform font-cinzel text-sm sm:text-lg tracking-widest backdrop-blur-sm ${
                activeCategory === cat
                  ? 'bg-gradient-to-br from-amber-900/80 to-rose-700/80 border-rose-800 scale-105 shadow-xl shadow-amber-900/30'
                  : 'bg-amber-900/20 border-amber-800/30 text-rose-100/80 hover:bg-rose-800/40 hover:scale-95'
              }`}
            >
              {cat}
            </button>
